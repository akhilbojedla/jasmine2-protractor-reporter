...
function Jasmine2ScreenShotReporter(opts) {
        ...
        '<html>' +
            '<head>' +
                ...
                '<style>' +
                    ...
                    'ul { list-style-position: inside; margin: 10px; padding: 5px; border: 1px solid #ABC; border-radius: 5px;border-left: 6px solid blue;}' +
                    'li {margin: 10px;padding: 5px;border: 1px solid #ABC;border-radius: 5px;border-left: 6px solid blue;}'+
                    'li[class=passed] {background-color: hsla(120, 100%, 75%, 0.3);border-left: 6px solid green;}' +
                    'li[class=failed] {background-color: hsla(0, 100%, 75%, 0.3);border-left: 6px solid red;}' +
                    'li[id=summaryTotalFailed] {color: red;}' +
                    'h4 {cursor: pointer;}' +
                ...
                '$(\'li[class=passed]\').hide();' +
                '$(\'li[class=failed]\').hide();' +
                '$(\'h4\').click(function(){' +
                'var element = $(this).parent();' +
                'element.find(\'li[class=passed]\').toggle(\'fast\');' +
                'element.find(\'li[class=failed]\').toggle(\'fast\');' +
                '});' +
                '});' +
                '</script>' +
            '</head>' +
    ...
    var addReportSummary = _.template(
      '<div id="summary" class="passed">' +
        '<h4><span class="summary">&#9997;</span>Summary</h4>' +
        '<ul>' +
        ...
        '<%= quickLinks %>' +
      '</div>' +
      '<h4><span class="summary">&#10002;</span> Test Results</h4>'
    );
    ...
    this.suiteStarted = function(suite) {
        ...
        if (runningSuite) {
            ...
            console.log("\nStarted Suite : " + suite.description + "\n");
        }
    ...
    this.suiteDone = function(suite) {
        ...
        console.log("\nFinished suite : " + suite.description + "\n");
    ...
    this.specStarted = function(spec) {
        ...
        console.log('\tStarted : ' + spec.description);
    };
      ...
      _.each(browser.forkedInstances, function (browserInstance, key) {
        ...
        browserInstance.takeScreenshot().then(function (png) {
          browserInstance.getCapabilities().then(function (capabilities) {
            ...
            console.log("\tFinished : " + spec.description + " -------> " + spec.status);
    ...
    function printResults(suite) {
        ...
        output += '<h4 calss="suite">' + suite.description + ' (' + getDuration(suite) + ' s)</h4>';
